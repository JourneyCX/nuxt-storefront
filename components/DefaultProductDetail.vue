...
// Standard product detail page, rendered when a tenant has no theme-driven
// "product" template configured (see pages/product/[slug].vue) — unchanged
// from the page this was extracted from, just moved into its own component so
...
// excluding the product being viewed. Ported from storefront/ProductDetail.vue
// (the Puck-widget counterpart) -- see that file for the same logic with a
// merchant-configurable on/off toggle; this page has no such settings surface,
// so it's unconditionally on.
...
<template>
  <div style="max-width:1200px;margin:0 auto;padding:48px 24px">
    ...
    <!-- You may also like -->
    <section v-if="relatedProducts && relatedProducts.length > 0" style="margin-top:64px;border-top:1px solid #e2e8f0;padding-top:48px">
      <h2 style="margin:0 0 24px;font-size:24px;font-weight:700;color:#1a202c">You May Also Like</h2>
