...
// Same carousel chrome as ProductCarousel.vue, but sourced from a pinned
// collection (via /api/collections/{slug}, same endpoint CollectionDetail.vue
// uses) instead of a category filter — mirrors studio-app's CollectionCarousel.tsx.
...
const products = computed(() => (data.value?.products ?? []).slice(0, clamp.value))
const showCarousel = computed(() =>
  !pending.value && !!props.collectionSlug && products.value.length > 0
)
const isEmpty = computed(() =>
  !pending.value && !!props.collectionSlug && !error.value && data.value !== null && products.value.length === 0
)
...
<template>
  <section :style="{ backgroundColor: backgroundColor || '#fff', padding: '56px 0' }">
    ...
    <div style="padding-left:24px;overflow-x:auto;">
      ...
      <div v-if="!collectionSlug" style="margin-right:24px;border:2px dashed #e2e8f0;border-radius:8px;padding:40px;text-align:center;color:#a0aec0;font-size:14px;">
        No collection selected.
      </div>

      <div v-else ref="trackRef" style="display:flex;gap:16px;padding-right:24px;padding-bottom:4px;">
        <!-- Loading skeleton -->
        ...
        <!-- Real products from the collection -->
          ...
          <div v-for="product in products" :key="product.id"
            ...
            <a :href="`/product/${product.slug}${previewSuffix}`" style="display:block;">
              <div style="position:relative;width:100%;aspect-ratio:1/1;background-color:#e2e8f0;display:flex;align-items:center;justify-content:center;overflow:hidden;">
                ...
                <span v-if="showBadge && badgeText" :style="{ position: 'absolute', top: '10px', left: '10px', backgroundColor: badgeColor || '#ef4444', color: '#fff', fontSize: '10px', fontWeight: 700, padding: '3px 8px', borderRadius: '4px', letterSpacing: '0.05em', textTransform: 'uppercase' }">
                  {{ badgeText }}
                </span>
              </div>
            </a>
        ...
        <!-- Placeholder fallback (no real products in the collection yet) -->
        <template v-else-if="showPlaceholder !== false && !isEmpty">
          <div v-for="i in clamp" :key="i"
            :style="{ width: `${cardWidth || 220}px`, flexShrink: 0, backgroundColor: '#fff', borderRadius: `${cardRadius || 12}px`, overflow: 'hidden', boxShadow: '0 1px 6px rgba(0,0,0,0.07)', border: '1px solid #f1f5f9' }">
        ...
        <div v-else style="padding:32px;color:#a0aec0;font-size:14px;">This collection has no products yet.</div>
      </div>
