...
function socialIconPath(platform: string): string | undefined {
  return SOCIAL_ICON_PATHS[resolvedPlatform(platform)]
}
      ...
      >
        ...
        <div v-if="hasSocial" style="display:flex;gap:12px;align-items:flex-start">
          <a
            ...
            :style="{ display:'inline-flex', alignItems:'center', textDecoration:'none', color: settings.footerTextColor||'#a0aec0' }"
          >
            <svg v-if="socialIconPath(social.platform)" viewBox="0 0 24 24" width="18" height="18" fill="currentColor" aria-hidden="true">
